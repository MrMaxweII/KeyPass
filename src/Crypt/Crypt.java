...
/****************************************************************************************************************************************************
*	Version 1.3    										Autor: Mr. Maxwell   												vom 24.02.2023			*
*																																					*
*	Letzte Änderung: (v1.3)																															*
*	Abhängikeit zu Bouncycastle entfernt!																											*
*	ParanoidCrypt wurde (ab KeyPass-Verion: 1.2.x) ohne Bouncycastle-Lib programmiert. 																*
*	Die verwendeten Klassen für AES, Twofis, Scrypt und SHA256 wurden in BTClib3001 hinzugefügt.													*
*																																					*
*	Letzte Änderung: (v1.2)																															*
*	in "encrypt()" und "decrypt()" stand bei: "new SecretKeySpec()" als Paramter noch "thwofish" obwohl es Variabel "name" sein sollte.				*
* 	Dieser Parameter hat zwar keien Auswirkungen bei Tests ergeben, aber es sollte dennoch richtig sein,											*
* 	denn es würde so aussehen, als ob fest mit "twofish" verschlülsselt wird, was nicht so ist, weil dies in einer Methode vorher festgelegt wird!	*
* 	Falls es dennoch später zu fehlern bei der Ver-Endschlüsselung kommen sollten, kann das hier wieder korrigiert werden.							*
*																																					*
...
*	Wird verwendet für 																																*
*	- CAGWallet.dat vom CoinAddressGernerator 																										*
*	- und für den KeyPass																															*
*	Achtung, für den KeyPass wurde ab version 1.1 in die Methode getScryptHash() der Parameter "memory"												*
*	auf einen niedrigeren Wert verändert um die Ver/Entschlüsselungs-Zeit zu verringern.															*
*	Dies macht die Verschlüsselung Inkompatibel zu anderen Versionen!																				*
*	Daher kann/darf diese Klasse nicht mit/in anderen Programmen ausgetauscht werden!!!																*
*****************************************************************************************************************************************************/
...
// -------------------------------------------- Test --------------------------------------------------------------
	...
	/** Test paranoidCrypt ausführlich in dem eine große Anzahl Testvektoren erzeugt, verschlüsselt und wieder entschlüsselt werden.
	*	Benchmarks:
	*	- Alt Bouncycastle Implementierung: für 10 Runden = 29 sec.
	*	- Neue Implementierung              für 10 Runden = 39 sec.
	**/
//	public static void main(String[] args) throws Exception
//	{
//	
//		final int testRounds = 10000;
//		
...
//			byte[] text = new byte[rand.nextInt(1000)];
//			rand.nextBytes(key);
//			rand.nextBytes(text);
//			text = padding16(text);
////			Out.print("key:  "); Out.hexPrintln(key);
////			Out.print("text: "); Out.hexPrintln(text);
//			byte[] chiffre = paranoidEncrypt(text, key);
////			Out.print("chiffre: "); Out.hexPrintln(chiffre);		
//			byte[] reText  = paranoidDecrypt(chiffre, key);
//			reText = removeAndCheckSHA256Checksum(reText)[1];			
//			if(Arrays.equals(text, reText)==false)
//			{
//				Out.println("Fehler! Entschlüsselter Text entspricht nicht dem original Text!");
//				Out.print("key:       "); Out.hexPrintln(key);
//				Out.print("text orig: "); Out.hexPrintln(text);
//				Out.print("chiffre:   "); Out.hexPrintln(chiffre);
//				Out.print("text re:   "); Out.hexPrintln(reText);
//				return;
//			}		
//			Out.println(i);
//		}
//		Out.println("Erfolg, alles wurde richtig ver- und entschlüsselt.");
//		Out.println("Zeit für "+testRounds+" Runden: "+((System.currentTimeMillis()-t)/1000)+"s");
//	}
...
/**	Verschlüsselt Daten mit zwei verschiedenen Verschlüsselungsmethoden hintereinander, AES & Twofish.
	Siehe Blockdiagramm "ParanoidCrypt.png
...
/**	Entschlüsselt Chiffre mit zwei verschiedenen Endschlüsselungsmethoden hintereinander, Twofish & AES
	Siehe Blockdiagramm "ParanoidCrypt.png
...
/**	Generiert einen Scrypt-Hash (RFC 7914) der zur Laufzeitverlängerung geeignet ist.
	Achtung hierfür wird ein eigener Thread gestartet!
...
private static byte[] getScryptHash(byte[] data) throws Exception
{
	...
	final int memory 	= 131072;	// (N) CPU/Speicherkostenparameter in Byte. Muss größer als 1 sein und eine Potenz von 2 und kleiner als 2^(128*r/8)  Standart = 2^14, Hier verwendet 2^17
...
/**	Hängt einen SHA256 Hash von "data" vorne an "data" an. 
	"Vorne" weil die hinten angehängten Leerzeichen des Paddings das hintere Parsen erschweren würden. 
...
// ----------------------------------------------------------- Alte Methoden mit BouncyCaslte Lib -------------------------------------------------------------
...
//	cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, name), new IvParameterSpec(iv));	
...
//	chiffre = Arrays.copyOfRange(chiffre, 16, chiffre.length);
//	Cipher cipher = Cipher.getInstance(name+"/CBC/noPadding", new BouncyCastleProvider());
//	cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(key, name), new IvParameterSpec(iv));	
